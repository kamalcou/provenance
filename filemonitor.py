...
from inspect import currentframe, getframeinfo  # for the line number printing
cf = currentframe()
...
def submitTransaction(filename, mode,sessionid,pid,username,timestamp1,timestamp2,cpu,operation,image_file_checksum,input_file_checksum=0,commanline_str=""):
    ...
    merged_commdline = commanline_str.replace(" ", "•")
    str+='"cmdline":"'+merged_commdline+'"}'

    # str+='"cmdline":"mpirun -np 16 apptainer run --bind result:/opt/result"}'
    # print("commanline_str: ", type(commanline_str))
...
def main():
    ...
    try:
        ...
        def print_event(cpu, data, size):
            ...
            event = b["events"].event(data)
            global mod_dict,pairs,commanline_str
            return_val=0
            ...
            if pairs.get(event.pid) == 1:
                if event.pid not in mod_dict[filename]:
                    ...
                    print("Event id: ",event.pid," cmdline: ", cmdline)
                   
                    commanline_str = cmdline[0]
            ...
            if pid_does_not_exist(event.pid) and pairs.get(event.pid) == 0:    
                ...
                submitTransaction(filename, mode1,event.sessionid,event.pid,username1,start_access,end_access,cpu,program_name,mod_dict[filename][event.pid].get('image_file_checksum'),mod_dict[filename][event.pid].get('input_file_checksum'),commanline_str)       
                with pair_lock:
                    pairs[event.pid] = 2
