...
output_file = "cached_cmdlines.log"
...
def get_cmdline(pid):
    try:
        with open(f"/proc/{pid}/cmdline", "rb") as f:
            ...
            return [a.decode('utf-8') for a in args if a]
    except:
        return ["[Exited]"]   
...
def safe_get_cmdline(pid, default="UNKNOWN"):
    try:
        ...
        return default
def write_to_file(pid, cmdline_list):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(output_file, "a") as f:
        cmdline_str = " ".join(cmdline_list)
        f.write(f"[{timestamp}] PID {pid}: {cmdline_str}\n")
...
def main():
    ...
    try:
        ...
        def print_event(cpu, data, size):
            ...
            # with cmd_lock:
            #     if event.pid not in cmds:
            #         cmds[event.pid] = get_cmdline(event.pid)
            #         write_to_file(event.pid, cmds[event.pid])
            ...
            if event.pid not in pairs:
                with pair_lock:
                    pairs[event.pid] = 1
                    # cmds[event.pid] =get_full_command(event.pid)
                # print("cmds: ", cmds)
            # print(psutil.Process(event.pid).cmdline())
            ...
            if pairs.get(event.pid) == 1:
                if event.pid not in mod_dict[filename]:
                    ...
                    with pair_lock:
                        pairs[event.pid]=0            
                    print("Match found")
                    # cmdline = cmds.get(event.pid, [])  # capture first
                    # print("cmdline: ", cmdline)
