...
#             # print(mod_dict)
#             return 1
def update_timestamp(filename, sessionid,pid, start_access, end_access=0,image_file_checksum=0,input_file_checksum=0):
	
    global mod_dict
    print(json.dumps(mod_dict, indent=2))
    if((filename not in mod_dict)):
        
        mod_dict[filename].update({'sessionid':sessionid,'pid':pid, 'start_access':start_access,'end_access':end_access,'image_file_checksum': image_file_checksum,'input_file_checksum': input_file_checksum,"active":1})
        #mod_dict[filename][comm][pid]=mode
        ...
        if(mod_dict[filename]['pid']==pid and mod_dict[filename]['end_access']!=end_access): 
            ...
            if(end_access-mod_dict[filename]['end_access']>=5):
                mod_dict[filename].update({'sessionid':sessionid,'pid':pid,'start_access': mod_dict[filename]['start_access'],'end_access':end_access,'image_file_checksum': mod_dict[filename]['image_file_checksum'],'input_file_checksum': mod_dict[filename]['input_file_checksum'],'active':mod_dict[filename]['active']})
...
#         mod_dict.update({filename:{'sessionid':sessionid,'pid':pid,'start_access': mod_dict[filename]['start_access'],'end_access':end_access,'image_file_checksum': image_file_checksum,'input_file_checksum': mod_dict[filename]['input_file_checksum'],'active':mod_dict[filename]['active']}})
def update_image_file_checksum(filename, sessionid, pid, start_access, end_access, image_file_checksum, input_file_checksum=0):
    global mod_dict
    if image_file_checksum != 0:
        ...
        if filename in mod_dict:
            if(pid==mod_dict[filename].get('pid', 0)):
                mod_dict[filename].update({
                    'sessionid': sessionid,
                    'pid': pid,
                    'start_access': mod_dict[filename].get('start_access'),
                    'end_access': end_access,
                    'image_file_checksum': image_file_checksum,
                    'input_file_checksum': mod_dict[filename].get('input_file_checksum'),
                    'active': mod_dict[filename].get('active', 0)
                })            
...
def update_input_file_checksum(filename, sessionid,pid, start_access, end_access,image_file_checksum,input_file_checksum=0):
    ...
    if(mod_dict[filename]['pid']==pid and input_file_checksum!=0): 
        mod_dict[filename].update({'sessionid':sessionid,'pid':pid,'start_access': mod_dict[filename]['start_access'],'end_access':mod_dict[filename]['end_access'],'image_file_checksum': mod_dict[filename]['image_file_checksum'],'input_file_checksum': input_file_checksum,'active':mod_dict[filename]['active']})
            
def update_pid_active(filename, sessionid,pid, active=0):
	
    global mod_dict
    
    mod_dict[filename].update({'sessionid':sessionid,'pid':pid,'start_access': mod_dict[filename]['start_access'],'end_access':mod_dict[filename]['end_access'],'image_file_checksum': mod_dict[filename]['image_file_checksum'],'input_file_checksum': mod_dict[filename]['input_file_checksum'],'active':active})
        ...
        def print_event(cpu, data, size):
            ...
            if(return_val==1):
                ...
                if is_process_active(event.pid):
                    update_pid_active(filename, event.sessionid,event.pid, active=1) 
                ...
                if is_process_active(event.pid):
                    ...
                    if(mod_dict[filename]['pid']==event.pid and mod_dict[filename]['active']==1):
                    ...
                    update_pid_active(filename, event.sessionid,event.pid, active=0)
