...
def update_timestamp(filename, sessionid,pid, start_access, end_access,image_file_checksum=0,input_file_checksum=0):
    ...
    if pid not in mod_dict.get(filename, {}):
        # You can choose to initialize it here or just return 0
        print(f"PID {pid} not found in mod_dict for filename {filename}.")
        return 0
    if( mod_dict[filename][pid].get('end_access')!=end_access): 
        # print (mod_dict[filename]['start_access'],end_access)
        if(end_access-mod_dict[filename][pid].get('end_access')>=5):
...
def main():
    ...
    try:
        ...
        def print_event(cpu, data, size):
            ...
            # if not pair_key_exists(pairs, event.pid):
            # if event.pid not in pairs:
            #     pairs[event.pid] = 1
            
            # print(json.dumps(pairs, indent=2))
                # pairs.append((event.pid, 1))
            # list.append(event.pid)
            ...
            # if pairs.get(event.pid) == 1:
                if event.pid not in mod_dict[filename]:
                    ...
                    match = re.search(r'[\w\-/\.]*\.sif', cmdline[0])
                    print(match.group(0) if match else "No .sif file found")
                    image_file = match.group(0) if match else ""
                    match = re.search(r'[\w\-/\.]*\.bin', cmdline[0])
                    print(match.group(0) if match else "No .bin file found")
                    input_file = match.group(0) if match else ""
                    ...
                    if input_file!="":
                        ...
                        input_file_checksum = sha256_file(origin_folder+"/"+input_file)
                    else:
                        input_file_checksum=0
        ...
        # def lost_event_handler(cpu, count):
